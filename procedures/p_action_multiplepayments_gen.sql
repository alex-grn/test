 ...
 IF OB.RNUMBER = 1 THEN 
    ...
    IF (SELECT COUNT(*)
       ...
       WHERE B.BENEFITSRECIPIENTSID = REC.BENEFITSRECIPIENTSID
         ...
         AND S.ID = C.BENEFITCHILDID
         AND (S.DOCBIRTHCHILDNUMBER = REC.DOCBIRTHCHILDNUMBER or (S.DOCBIRTHCHILDNUMBER IS NULL AND REC.DOCBIRTHCHILDNUMBER IS NULL))
         AND (S.DOCBIRTHCHILDSERIAL = REC.DOCBIRTHCHILDSERIAL or (S.DOCBIRTHCHILDSERIAL IS NULL AND REC.DOCBIRTHCHILDSERIAL IS NULL))
         AND H.ID != REC.PAYSID
         AND P.REPYEAR = NYEAR

...
begin
  ...
  loop
    ...
    insert into CASHPAYMENT_HEADER
      (UID, LID, CASHDOCSID, DOCTYPEID, DOCSTATUS, DOCNUMB, DOCDATE, PERSONID, BASIS)
    values
      (NUID, P_SYSTEM_GEN_LID('CASHPAYMENT_HEADER',nUID,UNIT), NCASHDOCSID, NDOCTYPEID, '1', COALESCE(NEXT_NUM,1), DDATE, REC.PERSONID, 'Расчетно-платежная ведомость № ' || REC.DOCNUMB || ' от ' || REC.DOCDATE)

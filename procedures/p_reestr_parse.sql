            ...
            from file_imp f WHERE F.nzap = rec.nzap
                ...
                loop
                  ...
                  if dow.stable = 'BENEFITSRECIPIENTS' then
                    ...
                    if dow.col7::numeric <> dow.col6::numeric / dow.col1::integer then tRETURN = ltrim(tRETURN||chr(13)||'б№хфэхфѓјхтющ фюѕюф їыхэют ёхьќш эх ёююђтхђёђтѓхђ рыую№шђьѓ №рёїхђр. аххёђ№ эх чру№ѓцхэ!',chr(13));
                                                    delete from BENEFICIARIESREGISTERS s where s.id = (select max(x.id) from BENEFICIARIESREGISTERS x); 
                    ...
                    if dow.col3::numeric > 1.5 * nTEMP then tRETURN = ltrim(tRETURN||chr(13)||'б№хфэхфѓјхтющ фюѕюф їыхэют ёхьќш фюыцхэ сћђќ эх сюыќјх, їхь т 1,5 №рчр, №рчьх№р я№юцшђюїэюую ьшэшьѓьр ђ№ѓфюёяюёюсэюую эрёхыхэшџ. аххёђ№ эх чру№ѓцхэ!',chr(13));
                                                    delete from BENEFICIARIESREGISTERS s where s.id = (select max(x.id) from BENEFICIARIESREGISTERS x); 
                                            /*return tRETURN;*/ end if;

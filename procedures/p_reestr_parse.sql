                     ...
                     select s.id
                      ...
                      --
                       begin
                        ...
                        if dow.col9 <=0 then FL:=2; SERRORS := SERRORS||CHR(13)||SMESSAGE_ERR||'@'||temp||' дШЮ №хсхэър: '||COALESCE(dow.col1,'')||' '||COALESCE(dow.col2,'')||' '||COALESCE(dow.col3,'')||' ёх№шџ ш эюьх№ фюъѓьхэђр: '||COALESCE(dow.col6::text,'')||' '||COALESCE(dow.col7::text,'')||'. Юїх№хфэюёђќ №юцфхэшџ №хсхэър эх ьюцхђ сћђќ <=0 шыш яѓёђющ!'; end if;

...
$body$
 ...
 begin 
   ...
   --  if lower(snode) = 'benefitreceiver'          then insert into benefit07(hid)          VALUES(null); select max(s.id) into nID from benefit07 s;          end if;
   ...
   loop
     ...
     -- обработка значений атрибутов ...  
      if lower(snode) = 'header' then 
        ...
        --elsif attr.sprop = 'to' then tHead:=tHead||attr.svalue; 
        elsif attr.sprop = 'date'         then update BENEFICIARIESREGISTERS s set dateform = attr.svalue::date where s.id = (select max(id) from BENEFICIARIESREGISTERS);
           ...
           for rec in (select x.id as xid,reg.id as regid, lower(se.name) as fromm, se.code as sender, x.repyear, x.repmonth::integer as smonths 
                    	  ...
                    	  and se.id  = x.subjectsdirid
                  	      and reg.id = (select max(f.id) from BENEFICIARIESREGISTERS f) )
                  		 loop
                          if rec.sender <> tBUF[2]::integer THEN raise using message = 'Код отправителя не соответствует отправителю. Реестр не загружен.';
                          elsif rec.repyear <> tBUF[4]::integer THEN raise using message = 'Год отчетного периода в реестре не соответствует отчетному периоду. Реестр не загружен!';
                          elsif rec.smonths <> tBUF[3]::integer THEN raise using message = 'Месяц отчетного периода в реестре не соответствует отчетному периоду. Реестр не загружен.';
                          end if;flag:=1;
         ...
         -- raise using message = attr.sprop;
           if attr.sprop = 'lastname'  			  then tBUF[1]:=attr.svalue; 
                                                        ...
                                                        if flag <> 2 then raise using message = 'Категория получателя пособия не соответствует. Реестр не загружен!'; flag := 0; end if;
       ...
       --benefit07
           ...
           if attr.sprop = 'persondocumenttype'   then for rec in(select x.id
                                                        ...
                                                        if flag <> 2 then raise using message = 'Вид документа, удостоверяющего личность получателя, не соответствует. Реестр не загружен!'; flag := 0; end if;
       ...
       --   
        ...
        elsif attr.sprop = 'childnumber'		  then tBUF[9]:=attr.svalue; --benefit01
       end if;
     --benefit07
           ...
           if attr.sprop = 'childdocumenttype'	  then for rec in(select x.id
                                                        ...
                                                        if flag <> 2 then raise using message = 'Вид документа, подтверждающего факт рождения ребенка, не соответствует. Реестр не загружен!'; flag := 0; end if;
     ...
     --benefit01
        ...
        elsif attr.sprop = 'documentregistered' then tBUF[7]:=attr.svalue;
     ...
     --
        ...
        elsif attr.sprop = 'averageincome'	       then tBUF[7]:=replace(attr.svalue,',','.'); if replace(attr.svalue,',','.')::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if; 
       end if; 
     ...
     --benefit07
     	   ...
     	   if attr.sprop = 'subsistencechild'/*'subsistence'*/          then tBUF[1]:=replace(attr.svalue,',','.'); if tBUF[1] = '' then tBUF[1] = '0'; end if; if tBUF[1]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
    ...
    /* elsif lower(snode) = 'paymentperiod' then
        ...
        elsif attr.sprop = 'paysum' 	          then tBUF[4]:=replace(attr.svalue,',','.'); if tBUF[4] = '' then tBUF[4] = '0'; end if; if tBUF[4]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        end if;
        ...
        elsif attr.sprop = 'surchargesum' 	      then tBUF[7]:=replace(attr.svalue,',','.'); if tBUF[7] = '' then tBUF[7] = '0'; end if; if tBUF[7]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        end if;
        ...
        elsif attr.sprop = 'refundsum' 	          then tBUF[10]:=replace(attr.svalue,',','.'); if tBUF[10] = '' then tBUF[10] = '0'; end if; if tBUF[10]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        end if; 
        ...
        elsif attr.sprop = 'holdsum' 	          then tBUF[13]:=replace(attr.svalue,',','.'); if tBUF[13] = '' then tBUF[13] = '0'; end if; if tBUF[13]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        end if; 
     ...
     --benefit01
        ...
        elsif attr.sprop = 'sumtotal' 	  		  then tBUF[2]:=replace(attr.svalue,',','.'); if tBUF[2] = '' then tBUF[2] = '0'; end if; if tBUF[2]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        elsif attr.sprop = 'datepay' 	  	      then tBUF[3]:=attr.svalue;
        elsif attr.sprop = 'sumpay' 	  		  then tBUF[4]:=replace(attr.svalue,',','.'); if tBUF[4] = '' then tBUF[4] = '0'; end if; if tBUF[4]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        elsif attr.sprop = 'date' 				  then tBUF[5]:=attr.svalue;
        elsif attr.sprop = 'dateextra' 			  then tBUF[6]:=attr.svalue;
        elsif attr.sprop = 'extrapay' 			  then tBUF[7]:=replace(attr.svalue,',','.'); if tBUF[7] = '' then tBUF[7] = '0'; end if; if tBUF[7]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        elsif attr.sprop = 'datereturn' 		  then tBUF[8]:=attr.svalue;
        elsif attr.sprop = 'sumreturn' 			  then tBUF[9]:=replace(attr.svalue,',','.'); if tBUF[9] = '' then tBUF[9] = '0'; end if; if tBUF[9]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        elsif attr.sprop = 'dateretention' 		  then tBUF[10]:=attr.svalue;
        elsif attr.sprop = 'sumretention' 		  then tBUF[11]:=replace(attr.svalue,',','.'); if tBUF[11] = '' then tBUF[11] = '0'; end if; if tBUF[11]::numeric <= 0 then raise using message = 'Суммы при заполнении должны быть больше нуля. Реестр не загружен!'; end if;
        end if; 
     end if;
   end loop;
